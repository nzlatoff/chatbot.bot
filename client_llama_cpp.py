...
class SlidingWindowLLM:
    def __init__(self, model_path, max_context_size, temperature, top_p, top_k):
        """
        Initialise le modèle avec une fenêtre de contexte glissante.

        :param model_path: Chemin vers le modèle GGUF
        :param max_context_size: Nombre maximal de tokens en contexte
        ...
        self.top_p = top_p
        self.top_k = top_k
        self.context_tokens = []  # Stocke le contexte sous forme de tokens
    ...
    def generate(self, prompt_tokens, batch_size, max_tokens=170):
        ...
        # Convertit le prompt tokenizé (np.array) en liste pour le stocker dans le contexte local
    ...
    def decode(self, s):
        ...
        if isinstance(s[0], (int, np.integer)):
            return self.model.detokenize(s).decode("utf-8")
        elif isinstance(s, (list, tuple, np.ndarray)):
            return np.array([self.model.detokenize(ss).decode("utf-8") for ss in s])
...
def index_from_master():
    ...
    i = 0
    pprint("")
    print(f"**INDEX_FROM_MASTER BATCH_MSG_IND={BATCH_MSG_IND}")
...
def trim_tok(tkns):
    #print("TRIM_TOK -2- TOKS:\n", tkns)
    #print("TRIM_TOK -2- TEXT:\n", llm.decode(tkns))
    # print()
    #for i in {"\n","<|s|>", "<|e|>", " ", "<|endoftext|>"}:
    #    print(f"{repr(i)} -> {llm.model.tokenize(i.encode('utf-8'), add_bos=False)}")
    riddance = {
        #le_model.encode(i)[0]
...
def fancy_tok_typing(tkns):
    ...
    for i in range(nl_ind, total):
        ...
        prev = current
        #print(f"SEND_TYPING CHARACTER=/{char}/\nSEND_TYPING MESSAGE=/{message}/")
        send_typing(
...
def preprocess_prefix():
    ...
    if not args.character:
        ...
        char_encoded = np.array(llm.model.tokenize(("\n"+args.character).encode("utf-8"), add_bos=False), dtype=np.int32)
        with LeLocle:
    ...
    if TKNS.size > SEP_TKNS_LEN:
        ...
        pprint(f"TKNS={TKNS}", sp_aft=True)
        pprint(f"... or, in other words: {llm.decode(TKNS).strip()}", sp_aft=True)
        pprint(f"(length: {end_pref})")
        pprint(f"(end_pref_orig={end_pref_orig}, end_pref={end_pref_orig}, end_pref_after_injections={end_pref_after_injections})")
    else:
...
def select_in_batch(data, chars, messages):
    ...
    pprint(f"(select answer in batch of {args.batch_size} different one(s), BATCH_MSG_IND={BATCH_MSG_IND})")
    if BATCH_MSG_IND == -1:
...
def trim_tokens(tkns, end_pref, end_pref_after_injections):
    if args.character:
        ...
        for tkn in tkns:
            ...
            tmp_seq = llm.model.detokenize(msg.tolist()).decode("utf-8")
            tmp_seq = tmp_seq.strip()
            generated.append(tmp_seq)
    else:
        trimmed = [tkns[end_pref:] for tkns in tkns]
        generated = [seq.strip() for seq in llm.decode(trimmed)]
        #generated = [seq.strip() for seq in llm.model.detokenize(trimmed).decode("utf-8")]
...
def extract_chars_msgs(generated, data):
    ...
    for i, g in enumerate(generated):
        ...
        # ADD REGEX FOR SEPARATORS !!!!! CAREFUL
        message = regex.sub(r"<\|[es]\|>\s*\n?", "", message)
...
def generate_new():
    ...
    try:
        pprint(
                f"(Danger! {args.server_name} is about to think: generate!)",
            sep="-",
            sp_bf=True,
            sp_aft=True,
        )
    ...
    #print(f"end_pref={end_pref}/end_pref_after_injections={end_pref_after_injections}")
    ...
    pprint(f"(sending {args.batch_size} batch of information, seconds={args.wait}, BATCH_MSG_IND={BATCH_MSG_IND}) ")
    send_batch(
        {
            "chars": chars,
            "messages": messages,
...
def on_chat_message(data):
            ...
            with LeLocle:
                if args.mode == "legacy":
                    ...
                    RECEIVED_MSGS = np.concatenate(
                        ...
                        (RECEIVED_MSGS, np.array(llm.model.tokenize(("\n"+character).encode("utf-8"), add_bos=False), dtype=np.int32))
                    )
                    if message:
                        RECEIVED_MSGS = np.concatenate(
                            #(RECEIVED_MSGS, le_model.encode(f"\n{message}"), SEP_TKNS)
                            (RECEIVED_MSGS, np.array(llm.model.tokenize(("\n"+message).encode("utf-8"), add_bos=False), dtype=np.int32), SEP_TKNS)
                        )
                    else:
            ...
            with LeLocle:
                if args.mode == "legacy":
                    ...
                    RECEIVED_MSGS = np.concatenate(
                        ...
                        (RECEIVED_MSGS, np.array(llm.model.tokenize(message.encode("utf-8"), add_bos=False), dtype=np.int32), SEP_TKNS)
                    )
        else:
            if args.mode == "legacy":
                if len(PREFIX) == 0:
    ...
    #print(TKNS, type(TKNS))
    #pprint("(after reception, TKNS-TEXT is:)", sep="-", sp_bf=True)
    #print("/"+llm.model.detokenize(TKNS.tolist()).decode('utf-8')+"/")
    #pprint(llm.decode(TKNS), sp_aft=True)
    #print(f"RECEIVED_MSGS=/{RECEIVED_MSGS}/")
    #print(f"RECEIVED_MSGS(TEXT)=/{llm.model.detokenize(RECEIVED_MSGS.tolist()).decode('utf-8')}/")
